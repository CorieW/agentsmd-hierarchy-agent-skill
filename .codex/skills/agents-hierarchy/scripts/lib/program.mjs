...
function addSharedDebugOption(command) {
  return command.option('--debug', 'Enable structured debug logs for testing and troubleshooting.');
}
...
export function createProgram(runtime = {}) {
  ...
  program
    ...
    .description('Validate, scaffold, and install the AGENTS Hierarchy skill bundle.')
    .showHelpAfterError()
    .exitOverride();

  addValidateOptions(
    program.command('check [path]').description('Validate AGENTS.md files without changing them.')
  ...
  addValidateOptions(
    program.command('fix [path]').description('Refresh missing or stale AGENTS.md files.')
  ).action(async (targetPath, options) => {
    const argv = ['--fix'];
    if (targetPath) {
      argv.push(targetPath);
  ...
  addSharedDebugOption(
    program
      ...
      .description('Scaffold AGENTS.md files for a required repo-relative directory.')
    ...
    const exitCode = await runValidateAgentsCommand(argv, sharedRuntime);
    if (exitCode !== 0) {
      throw new CommanderError(exitCode, 'agents-hierarchy.scaffold.failed', '');
    }
  });

  addSharedDebugOption(
    program.command('install').description('Install the skill bundle into Codex, Claude, Cursor, or a Codex plugin directory.')
  )
    .addOption(new Option('--tool <tool>').choices(['codex', 'claude', 'cursor']))
    .addOption(new Option('--scope <scope>').choices(['personal', 'project']))
    .addOption(new Option('--mode <mode>').choices(['skill', 'plugin', 'command']))
    .option('--project-root <path>', 'Project root to use for project-scoped installs.')
    ...
    .option('--no-prompt', 'Disable interactive prompts and require deterministic inputs.')
    .action(async options => {
      const exitCode = await runInstallCommand(options, sharedRuntime);
      if (exitCode !== 0) {
        throw new CommanderError(exitCode, 'agents-hierarchy.install.failed', '');
      }
    });
